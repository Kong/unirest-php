...
class Unirest {
	...
	/**
	 * SSL verify doesnot work without specifying the PEM file for the certificate
	 * @param string $filepath ,empty by defualt
	 */
	public static function sslPemFile($filepath) {
		Unirest::$sslPemFile = $filepath;
	...
	private static function request($httpMethod, $url, $body = NULL, $headers = array(), $username = NULL, $password = NULL) {
		...
		if (Unirest::$verifyPeer) {
			if (file_exists(Unirest::$sslPemFile)) {
				curl_setopt($ch, CURLOPT_CAINFO, Unirest::$sslPemFile);
